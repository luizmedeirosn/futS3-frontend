...
export class EditPlayerFormComponent implements OnInit, OnDestroy {
  ...
  public handleSelectPlayer(id: number): void {
    if (id) {
      ...
      const selectedPlayer: Observable<PlayerDTO> = this.playerService.findById(id);
      const combined$: Observable<[Page<ParameterDTO>, PlayerDTO]> = zip(totalParameters$, selectedPlayer);
