...
export class EditPositionFormComponent implements OnInit, OnDestroy {
  ...
  public handleSelectPosition(id: number): void {
    if (id) {
      // Reset available parameters whenever a new position is chosen due to the strategy of deleting parameters that already belong to the selected position
      const totalParameters$: Observable<Page<ParameterDTO>> = this.parameterService.findAllWithTotalRecords();

      const selectedPosition$: Observable<PositionDTO> = this.positionService.findById(id);
      const combined$: Observable<[Page<ParameterDTO>, PositionDTO]> = zip(totalParameters$, selectedPosition$);

      // It's necessary to synchronize the requests to avoid issues with undefined in 'this.totalParameters' in the 'deleteIncludedPositionParameters' method
  ...
  public handleSubmitEditPositionForm(): void {
    if (this.editPositionForm.valid && this.editPositionForm.value) {
      ...
      this.selectedPosition &&
        this.positionService
          .updateById(this.selectedPosition?.id, positionRequest)
          .pipe(takeUntil(this.destroy$))
          .subscribe({
            next: () => {
